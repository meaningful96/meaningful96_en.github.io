...
The first step is the training process, which includes 1) **sparsification**. This is a preparation step for the subsequent pruning process. Pruning requires the ability to assess weights. Weight assessment means retaining only the most important parameters during training while making the others small. To achieve this, L1 Regularization and Entropy Regularization are added to the loss function.
